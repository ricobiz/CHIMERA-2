...
- ВСЕГДА сначала пробуй БЕЗ телефона, даже на строгих сайтах (Gmail, Facebook)
- Если задача = регистрация → попробуй заполнить форму, затем если потребуется телефон - остановись (WAIT_USER)
- Если задача = простая навигация → прогрев не нужен
- strategy должна быть "attempt_without_phone" в 95% случаев, только если явно указано что нужен телефон - тогда "require_phone"
...
Проанализируй и создай стратегию."""
        ...
        try:
            ...
            if user_data:
                ...
                data_bundle = {
                    ...
                    "address": user_data.get('address', realistic['address']),
                    "city": user_data.get('city', realistic['city']),
                    "state": user_data.get('state', realistic['state']),
                    "country": user_data.get('country', realistic['country']),
                    "postal_code": user_data.get('postal_code', realistic['postal_code']),
                }
                data_source = "user_provided"
                logger.info(f"✅ [HEAD BRAIN] Using user-provided data (filled {sum(1 for v in user_data.values() if v)} fields)")
            else:
                # Полностью генерируем РЕАЛИСТИЧНЫЕ данные
                realistic = _gen_realistic_data()
                data_bundle = realistic
                logger.info("✅ [HEAD BRAIN] Generated realistic data automatically")
    ...
    def _fallback_analysis(self, goal: str, has_warm_profile: bool, user_data: Optional[Dict] = None, auto_generate: bool = False) -> Dict[str, Any]:
