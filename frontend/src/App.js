...
function App() {
  ...
  // Handle design approval and annotation
  const handleAnnotateMockup = (img) => {
  ...
  const handleApproveDesign = async () => {
    ...
    console.log('🎯 Approving design with:', { prompt, designSpec: designSpec?.substring(0, 100) });
    
    if (!prompt || !designSpec) {
      console.error('❌ Missing prompt or design spec');
      toast({ title: 'Error', description: 'Missing design information', variant: 'destructive' });
      return;
    }
  ...
  // Wire annotator callbacks for ChatInterface (kept for backwards compatibility)
  useEffect(() => {
    window.onAnnotateMockup = handleAnnotateMockup;
    window.onApproveDesign = handleApproveDesign;
    return () => {
      window.onAnnotateMockup = null;
      window.onApproveDesign = null;
    };
  }, [pendingPrompt, pendingDesign, messages, totalCost, currentSessionId, selectedModel]);
