...
const AIEntryPoint = ({ onClose }) => {
  ...
  const [lastAutomation, setLastAutomation] = useState(null); // { sessionId, screenshot_base64, grid, vision }
  ...
  // Scrolling and polling refs
  const logsEndRef = useRef(null);
  const pollIntervalRef = useRef(null);

  // Smooth screenshot swap with preload
  useEffect(() => {
  ...
  const loadLogs = async () => {
    try {
      ...
      if (data.profile_id || profileHealth.profile_id) {
        setProfileHealth(ph => ({ ...ph, profile_id: data.profile_id || ph.profile_id }));
      }
    } catch (e) {
  ...
  useEffect(() => {
    if (autoRefresh) {
      pollIntervalRef.current = setInterval(loadLogs, 1500);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setCurrentTask(t.text || '');
      } catch {}
      try {
        const s = await getAgentStatus();
        setAgentStatus(s.status || 'IDLE');
        setRunMode(s.run_mode || 'PAUSED');
      } catch {}
      try {
  ...
  const generateSessionLink = () => {
    ...
    const link = `${window.location.origin}/ai-entry?session=${sid}`;
    setSessionLink(link);
  };

  const toggleAccess = () => {
    setAccessEnabled(v => !v);
    if (!accessEnabled) generateSessionLink();
  };
  ...
  const startLiveSession = async () => {
    try {
      ...
      await automationCreateSession(sid, true);
      await automationNavigate(sid, 'https://accounts.google.com/signup');
      const shot = await automationScreenshot(sid);
      setLogs(prev => [...prev, { ts: Date.now(), step: prev.length + 1, action: 'Session initialized & navigated', status: 'ok' }]);
      setLastAutomation({ sessionId: sid, ...shot });
      setPendingSrc(shot.screenshot_base64);
    } catch (e) {
      alert('Automation init failed: ' + e.message);
    }
  };

  // Helper: position for ghost cursor
  ...
  const getStatusColor = () => {
    switch (agentStatus) {
      ...
      case 'WAITING_USER': return 'text-blue-400';
      ...
      <div className="px-4 md:px-6 py-4 border-b border-gray-800 flex-shrink-0">
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={startLiveSession} className="px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-600/40 text-blue-400 rounded text-xs">Start Session</button>
          </div>
              ...
              <div ref={viewerRef} className="relative w-full h-72 md:h-[420px] flex items-center justify-center bg-black">
                ...
                {showGrid && lastAutomation?.grid && (
                  <div className="absolute inset-0 pointer-events-none">
                    <div className="w-full h-full" style={{ backgroundImage: `linear-gradient(rgba(200,200,200,0.08) 1px, transparent 1px), linear-gradient(90deg, rgba(200,200,200,0.08) 1px, transparent 1px)`, backgroundSize: `${(100/(lastAutomation.grid.cols||8))}% ${(100/(lastAutomation.grid.rows||12))}%` }} />
                  </div>
                )}
                {(() => {
                  // Find last meaningful step with a CELL
      ...
      <div className="flex-1 overflow-y-auto flex flex-col lg:flex-row gap-4 p-4 md:p-6">
        ...
        <div className="lg:w-1/3 space-y-4">
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={accessEnabled} onChange={() => toggleAccess()} className="sr-only peer" />
            ...
            {accessEnabled ? (
              <>
                <p className="text-xs text-gray-500 mb-3">Поделитесь этой ссылкой с внешним AI для доступа к агенту:</p>
                <div className="bg-gray-800/50 border border-gray-700 rounded p-2 mb-2 flex items-center gap-2">
                  <input type="text" value={sessionLink} readOnly className="flex-1 bg-transparent text-xs text-gray-300 outline-none" />
                  <button onClick={copyLink} className="px-2 py-1 bg-purple-600/20 hover:bg-purple-600/30 border border-purple-600/50 text-purple-400 rounded text-xs transition-colors">Copy</button>
                </div>
                <p className="text-xs text-green-400">✓ Доступ активирован для этой сессии</p>
              </>
            ) : (
              <p className="text-xs text-gray-500">Включите переключатель, чтобы сгенерировать ссылку для внешнего AI.</p>
            )}
          </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={async () => {
                  const profId = profileHealth.profile_id || prompt('Profile ID to re-check:');
                  if (!profId) return;
                  try {
                    const resp = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/profile/check`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile_id: profId }) });
                    const data = await resp.json();
                    alert(`Re-check done. Clean=${data.is_clean}`);
                    setProfileHealth(ph => ({ ...ph, is_clean: data.is_clean }));
                  } catch (e) {
                    alert('Re-check failed: ' + e.message);
                  }
                }} className="px-3 py-1.5 bg-purple-600/20 hover:bg-purple-600/30 border border-purple-600/50 text-purple-300 rounded text-xs">Re-check profile now</button>
              </div>
            </div>
            <div className="text-xs text-gray-400 space-y-1">
              <div>profile_id: <span className="text-gray-200">{profileHealth.profile_id || '—'}</span></div>
              <div>warm: <span className="text-gray-200">{profileHealth.is_warm ? 'yes' : 'no'}</span></div>
              <div>clean: <span className="text-gray-200">{profileHealth.is_clean === null ? '—' : profileHealth.is_clean ? '✅' : '❌'}</span></div>
            </div>
          </div>
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            ...
            <div className={`text-2xl font-bold ${getStatusColor()}`}>{agentStatus}</div>
            <div className="mt-1 text-sm text-gray-500">Mode: <span className={runMode === 'ACTIVE' ? 'text-green-400' : 'text-gray-400'}>{runMode}</span></div>
          </div>
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            ...
            <div className="text-sm text-gray-300 break-words">{currentTask || <span className="text-gray-600 italic">No active task</span>}</div>
            {jobId && (<div className="mt-2 text-xs text-gray-500 font-mono">Job ID: {jobId}</div>)}
          </div>

          {/* Task Input */}
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            <label className="block text-sm text-gray-400 font-medium mb-3">Task Input</label>
            <textarea value={taskText} onChange={(e) => setTaskText(e.target.value)} placeholder={`Enter task in natural language...

Example: Register new Gmail account, bypass CAPTCHA, return login/password and confirmation screenshot`} className="w-full h-32 bg-gray-950 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-blue-500 resize-none" />
            <button onClick={handleRunTask} disabled={isSubmitting || !taskText.trim()} className="mt-3 w-full px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-2">
              <button onClick={handleForceExecute} className="w-full px-3 py-2 bg-red-600/20 hover:bg-red-600/30 border border-red-600/50 text-red-400 rounded-lg text-sm font-medium transition-colors">💥 Force Execute</button>
              <button onClick={handleForceRefresh} className="w-full px-3 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-600/50 text-blue-400 rounded-lg text-sm font-medium transition-colors">🔁 Force Refresh</button>
              <button onClick={handleReadLogs} className="w-full px-3 py-2 bg-green-600/20 hover:bg-green-600/30 border border-green-600/50 text-green-400 rounded-lg text-sm font-medium transition-colors">📊 Read Logs</button>
            </div>
          </div>

          {/* Result */}
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3"><label className="text-sm text-gray-400 font-medium">RESULT</label>{result?.completed && <CheckCircle className="w-4 h-4 text-green-400" />}</div>
                ...
                {result.credentials && (
                  <div className="bg-gray-950 rounded p-3 border border-gray-700">
                    ...
                    <div className="space-y-1 text-sm font-mono">
                      <div className="flex items-center gap-2"><span className="text-gray-400">Login:</span><span className="text-green-400">{result.credentials.login}</span></div>
                      <div className="flex items-center gap-2"><span className="text-gray-400">Pass:</span><span className="text-green-400">{result.credentials.password}</span></div>
                    </div>
                  </div>
                )}
                {result.screenshot && (
                  <div className="bg-gray-950 rounded p-3 border border-gray-700">
                    <div className="text-xs text-gray-500 mb-2">Screenshot (base64):</div>
                    <div className="text-sm text-blue-400 font-mono break-all">{result.screenshot}</div>
                  </div>
                )}
                <button onClick={async() => { const r = await getResult(); if (r.result) setResult(r.result); }} className="w-full px-3 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-600/50 text-blue-400 rounded text-sm font-medium transition-colors">Refresh Result</button>
              </div>
            ) : (
              <div className="text-sm text-gray-600 italic text-center py-4">{result === null ? 'Final artifacts will appear here...' : 'Task in progress...'}</div>
            )}
          </div>
        </div>
        ...
        <div className="lg:w-2/3">
          <div className="bg-black border border-gray-800 rounded-lg h-full flex flex-col">
            <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
              <div className="flex items-center gap-2"><FileText className="w-4 h-4 text-green-400" /><span className="text-sm text-green-400 font-mono">EXECUTION LOG</span></div>
