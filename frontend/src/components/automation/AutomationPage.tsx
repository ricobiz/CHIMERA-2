    ...
    <div className="flex flex-col bg-[#0f0f10] text-gray-100 overflow-x-hidden" style={{ height: '100dvh' }}>
      ...
      {/* Top: Live Viewer */}
          ...
          <div className="absolute left-2 bottom-2 flex gap-2 z-20">
            <button onClick={async()=>{
              const url = prompt('Введите URL для входа', quickUrl) || quickUrl;
              setQuickUrl(url);
              await quickNavigate();
            }} className="px-2 py-1 text-[11px] bg-blue-900/70 hover:bg-blue-800/70 border border-blue-800 rounded text-blue-200">Открыть URL для входа</button>
            <button onClick={async()=>{
              if (!sessionId) { alert('Сессия не активна'); return; }
              try {
                const resp = await fetch(`${BASE_URL}/api/profile/save_from_session`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ session_id: sessionId }) });
                const d = await resp.json();
                if (d?.profile_id) alert('Профиль сохранён и прогрет'); else alert('Не удалось сохранить профиль');
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-800 flex-shrink-0">
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            <button onClick={async()=>{ try{ const resp = await fetch(`${BASE_URL}/api/automation/smoke-check`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ url: quickUrl, use_proxy:false }) }); const data: any = await resp.json(); if (data?.screenshot_base64) { setPendingSrc(data.screenshot_base64); setObservation(data); const v = (data.vision||[]) as any[]; lastSnapshotRef.current = { shotId: data.screenshot_id || null, vision: v, viewport: data.viewport || null, grid: data.grid || null }; setVision(v); } } catch(e:any){ alert(e.message||'Smoke failed');}}} className="px-2 py-2 text-[12px] bg-green-800/70 hover:bg-green-700/70 border border-green-700 rounded text-green-200 flex items-center justify-center gap-1"><BeakerIcon className="w-4 h-4"/><span>Smoke</span></button>
          </div>
        </div>
      </div>
