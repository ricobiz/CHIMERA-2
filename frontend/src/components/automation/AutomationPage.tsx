...
import type { BrowserState, AgentLogEntry } from '../../agent/types.ts';
    ...
    <div className="flex flex-col bg-[#0f0f10] text-gray-100 overflow-x-hidden" style={{ height: '100dvh' }}>
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-800 flex-shrink-0">
        <div ref={viewerRef} className="relative w-full h-72 md:h-[520px] border border-gray-800 rounded bg-black/60 overflow-hidden flex items-center justify-center" style={{ touchAction: 'none', overscrollBehavior: 'contain' }}>
          ...
          {/* Execution Subtitle - показывает текущее действие */}
          {executionSubtitle && isExecuting && (
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-40 max-w-md">
              <div className="px-4 py-2 bg-black/90 backdrop-blur-sm border border-blue-500/50 rounded-lg shadow-lg">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-blue-400 animate-spin" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <span className="text-sm text-gray-100">{executionSubtitle}</span>
                </div>
              </div>
            </div>
          )}
