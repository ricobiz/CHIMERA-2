      ...
      <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-800 flex-shrink-0">
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            <button onClick={quickCreate} className="px-2 py-2 text-[12px] bg-gray-800/70 hover:bg-gray-700/70 border border-gray-700 rounded text-gray-200 flex items-center justify-center gap-1"><PlusIcon className="w-4 h-4"/><span className="hidden sm:inline">New</span></button>
            <button onClick={quickNavigate} className="px-2 py-2 text-[12px] bg-blue-800/70 hover:bg-blue-700/70 border border-blue-700 rounded text-blue-200 flex items-center justify-center gap-1"><ArrowRightIcon className="w-4 h-4"/><span className="hidden sm:inline">Go</span></button>
            <button onClick={async()=>{ try{ const resp = await fetch(`${BASE_URL}/api/automation/smoke-check`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ url: quickUrl, use_proxy:false }) }); const data = await resp.json(); if (data?.screenshot_base64) setPendingSrc(data.screenshot_base64);} catch(e:any){ alert(e.message||'Smoke failed');}}} className="px-2 py-2 text-[12px] bg-green-800/70 hover:bg-green-700/70 border border-green-700 rounded text-green-200 flex items-center justify-center gap-1"><BeakerIcon className="w-4 h-4"/><span className="hidden sm:inline">Smoke</span></button>
          </div>
        </div>
      </div>
